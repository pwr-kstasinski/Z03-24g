...
class ChatGUI:
    ...
    def on_message(self, x, message):
        ...
        if message == "offline":
            ...
            #print("new message if")
            for chat in self.chats:
                chat.update_messages()
            self.update_users_list()
        elif message == "read":
            #print("try to update message status")
            for chat in self.chats:
                chat.message_read()
                #chat.update_messages()
            self.update_users_list()
    ...
    class ChatWindow(tk.Toplevel):
        ...
        def update_messages(self):
            ...
            try:
                messages = api.read_messages_to_user_from_date_message_receiver_id_sender_id_from_date_get(self.my_id,
                                                                                            self.receiver_id,
