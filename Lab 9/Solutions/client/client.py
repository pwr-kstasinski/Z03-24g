...
class UserWidget(QWidget, Ui_UserWidget):
    ...
    def setupUi(self, widget):
        ...
        converted_datetime = datetime.datetime.strptime(self.user.last_active, '%Y-%m-%d %H:%M:%S')
        time_difference = datetime.datetime.now() - converted_datetime
        seconds_difference = time_difference.total_seconds()
        if seconds_difference > MAX_ONLINE_DIFFERENCE:
...
class MessagesClient(Ui_MainWindow):
    ...
    def register_user(self):
        ...
        async_register_user(callback, name, password)
    ...
    def login_user(self):
        ...
        def login_user(data: dict, messages_data: dict):
        ...
        def check_password(data):
            ...
            if len(users_data) == 1:
                ...
                async_load_messages(lambda messages_data: login_user(data, messages_data), user)
            else:
                self.login_page_message_label.setText("Log in failed.")
                self.change_page_on_login()
                log.warning("log in failed")

        async_get_user_with_name(check_password, name)
