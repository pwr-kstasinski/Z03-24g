...
def get_messages_to_user_from_date(db: Session, receiver_id: int, date: datetime, sender_id: int, skip: int = 0, limit: int = 100):
    return db.query(models.Message) \
...
def login_user(db: Session, user: schemas.UserCreate):
    db_user = db.query(models.User).filter(models.User.login == user.login).filter(models.User.password == user.password).first()
    if db_user:
        return db_user
    return None
