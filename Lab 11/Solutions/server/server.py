...
def healthcheck():
    return json.dumps({'health_status': HEALTH_STATUS}), 200 if HEALTH_STATUS else 500


@app.route('/change_health')
def set_health_false():
    global HEALTH_STATUS
    HEALTH_STATUS = not HEALTH_STATUS
    return json.dumps({'health_status': HEALTH_STATUS}), 200
...
with app.app_context():
    ...
    print("Starting API: http://{}:{}{}".format(SELF_HOST, API_PORT, API_PREFIX))
    threading.Thread(target=lambda: app.run(host=SELF_HOST, port=API_PORT, debug=True, use_reloader=False)).start()

    print(f"Starting websocket: ws://{SELF_HOST}:{WS_PORT}")
    asyncio.get_event_loop().run_until_complete(websockets.serve(echo, SELF_HOST, 8765))
